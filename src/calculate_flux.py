...
def update_flux(s_vector):
    ...
    gv.c_in = (HEAT_CAPACITY_RATIO - 1) * 0.25 * (gv.u_infty - np.sqrt(np.power(gv.u[0,:,0], 2) + np.power(gv.u[0,:,1], 2))) + 0.5 * (gv.c_infty + gv.c[0,:])
    ...
    # West flux (fw) at the inlet cell (i = 0, j)
    gv.F[0, :, 3, 0] = gv.rho_in * gv.u_in * gv.ndS[0, :, 3, 0]                             # Mass flux (density)
    gv.F[0, :, 3, 1] = (gv.rho_in * np.power(gv.u_in, 2) + gv.p_in) * gv.ndS[0, :, 3, 0]    # Momentum flux in x-direction
    gv.F[0, :, 3, 2] = 0.0 # rho_in * u_in * v_in * gv.ndS[0, :, 3, 0]                      # Momentum flux in y-direction
    gv.F[0, :, 3, 3] = gv.rho_in * gv.u_in * gv.H_in * gv.ndS[0, :, 3, 0]                   # Energy flux in x-direction
    ...
    rho_out = gv.rho[-1, :] * np.power((gv.p_out[:] / gv.p[-1, :]), 1/HEAT_CAPACITY_RATIO)
    # Calculate outlet temperature (T_out)
    gv.T_out = gv.p_out/(rho_out * GAS_CONSTANT)
    ...
    H_out = SPECIFIC_HEAT_CP * gv.T_out + 0.5 * (np.power(gv.u_out, 2) + np.power(v_out, 2))
    # Calculate oulet mach number (M_out)
    gv.M_out = np.sqrt(np.power(gv.u_out,2) + np.power(v_out,2)) / gv.c_out

    # East flux (fe) at the outlet cell (i = NUM_CELLS_X-1, j)
    gv.F[-1, :, 1, 0] = rho_out * gv.u_out * gv.ndS[-1, :, 1, 0]                          # Mass flux (density)
    gv.F[-1, :, 1, 1] = (rho_out * np.power(gv.u_out, 2) + gv.p_out) * gv.ndS[-1, :, 1, 0]   # Momentum flux in x-direction
    gv.F[-1, :, 1, 2] = rho_out * gv.u_out * v_out * gv.ndS[-1, :, 1, 0]                  # Momentum flux in y-direction
    gv.F[-1, :, 1, 3] = rho_out * gv.u_out * H_out * gv.ndS[-1, :, 1, 0]                  # Energy flux in x-direction
    ...
    ad.update_artificial_dissipation(s_vector)
